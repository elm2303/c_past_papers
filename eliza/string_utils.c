...
char *empty_string(void) {
    char *str = malloc(1);
    if (str == NULL) {
        perror("empty_string() function failed");
        exit(EXIT_FAILURE);
    }
    str = "\0";
    return str;
...
char *clone(const char *str) {
    char *clonedStr = strcpy(malloc(strlen(str) + 1), str);
    if (clonedStr == NULL) {
        perror("clone() function failed");
        exit(EXIT_FAILURE);
    }
    return clonedStr;
}
...
char *push_string(char *current, const char *append) {
    int size = sizeof(current) + sizeof(append);
    char *str = malloc(size);
    if (str == NULL) {
        perror("push_string() function failed");
        exit(EXIT_FAILURE);
    }
    strcpy(str, current);
    strcat(str, append);
