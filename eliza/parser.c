...
int parse_eliza_script(struct eliza_state *eliza, const char *path) {
    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        if (strcmp(prefix, "initial") == 0) {
            ...
            char *val = empty_string();
            for (int index = 1; index < count; ++index) {
                val = push_string(val, tokens[index]);

                if (index + 1 < count)
                    val = push_string(val, " ");
            }

            const int inserted = map_insert(&eliza->prereplace, tokens[0], val);

            if (!inserted) { free(val); }
            ...
            const int inserted = map_insert(&eliza->postreplace, tokens[0], val);
